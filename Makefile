...
FETCH_URL=https://gperftools.googlecode.com/files/gperftools-2.1.tar.gz
POD_NAME=gperftools-2.1
...
build-source:
	@echo "\n Building $(POD_NAME) \n"
	@mkdir -p pod-build
	cd pod-build && ../$(POD_NAME)/configure --prefix=$(BUILD_PREFIX) --enable-minimal
	cd pod-build && make
	@touch built.touch
